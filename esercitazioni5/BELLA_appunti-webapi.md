...
## Seed/DataSeeder.cs (modificato):
Questo è il file più importante per i ruoli.

Con RoleManager crei i ruoli se mancano;

con UserManager crei gli utenti e gli assegni i ruoli.

L'uso di RoleManager per gestire i ruoli e UserManager.AddToRoleAsync per assegnare utenti ai ruoli è il pattern standard di Identity.
```c#
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;
using Rubrica.Api.Data;
...
`STAMPANDO SU GIT BASH IL RUOLO MI ESCE PER IL PROFILO AUTENTICATO EFFETTUANDO L'ACCESSO E ALLA QUALE VIENE FORNITO IL TOKEN`
